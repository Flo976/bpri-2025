...
Features:
 ...
 - Optionally pass in a config object with values like 
 	- draggable: true, 
 	- center: true, 
 	- speed (default: 1, which travels at roughly 100 pixels per second), 
 	- paused (boolean), 
 	- repeat, 
 	- reversed, and 
 	- paddingRight.
...
function HLoop(items, config) {
    ...
    config = config || {};
    let dragTriggerElement = items[0].parentNode;
    if (config.dragTrigger && items[0].closest(config.dragTrigger)) {
        dragTriggerElement = items[0].closest(config.dragTrigger);
    }
